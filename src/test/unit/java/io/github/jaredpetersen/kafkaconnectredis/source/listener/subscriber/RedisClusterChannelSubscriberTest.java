...
public class RedisClusterChannelSubscriberTest {
  @Test
  public void subscriberIssuesPsubscribeOnTopologyChange() {
    final String[] channels = new String[]{"podcasts", "books"};
    ...
    verify(redisConnectionMock.sync().upstream().commands()).subscribe(channels);
    verify(redisConnectionMock.sync().upstream().commands(), after(5000L).times(2)).subscribe(channels);
  }
}
